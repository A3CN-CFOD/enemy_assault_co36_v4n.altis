...
while { (alive _air1) and {canmove _air1} and {alive (driver _air1)} } do {
	...
	waituntil { (getSlingLoad _air1 == _liftObject) };
	...
	while {(_spawnmark distance2D _air1 > 100) and {!(getSlingLoad _air1 == _liftObject)}} do {
		if  !(getSlingLoad _air1 == _liftObject) then {
			_wp = [_airliftgrp,(getpos _liftObject), 5, "HOOK", "CARELESS", "NORMAL", "WEDGE"] call ghst_fnc_createwaypoint;
			_wp waypointAttachObject _liftObject;
		};
		sleep 5;
	};
