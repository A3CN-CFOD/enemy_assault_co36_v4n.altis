...
[_air1, "colorBLUFOR", "b_air", _trackname, true] spawn ghst_fnc_tracker;
...
_smoke = "SmokeShellGreen" createVehicle _pos;
	...
	if ((alive _air1) and {(canmove _air1)} and {(alive (driver _air1))}) then {
		...
		//ghst_dest_transport = [_air1,["<t size='1.5' shadow='2' color='#00FF00'>Helicopter Destination</t>", "call ghst_fnc_dest_transport", [_air1,_flyheight,_escortgrp], 5, false, true, "","alive _target"]] remoteExec ["addAction",0,true]; 
		...
		//ghst_rtb_transport = [_air1,["<t size='1.5' shadow='2' color='#00FFFF'>Helicopter RTB</t>", "call ghst_fnc_rtb_transport", [_air1,_flyheight,_spawnmark,_escortgrp,[_air1,_air2,_air3]], 5, false, true, "","alive _target"]] remoteExec ["addAction",0,true];
	};
