...
for "_i" from 0 to (count _cfgvehicles)-1 do {
	...
	if (isClass _vehicle) then {
		...
		if ((((_wCName iskindof "Helicopter_Base_F") or (_wCName iskindof "Plane_Base_F") or (_wCName iskindof "VTOL_01_base_F")) && ((_wside == 1) or (_wside == 2)) && (_wscope == 2) && ((_wfaction == "BLU_F") or (_wfaction == "rhs_faction_usarmy_wd") or (_wfaction == "rhs_faction_usarmy_d") or (_wfaction == "rhs_faction_usmc_wd") or (_wfaction == "rhs_faction_usmc_d") or (_wfaction == "rhs_faction_usaf") or (_wfaction == "rhs_faction_socom") or (_wfaction == "BLU_T_F")) && (_wDName!="") && !(_wCName iskindof "ParachuteBase") && !(_wCName iskindof "UAV_01_base_F") && !(_wCName iskindof "UAV_03_base_F") && (_wModel!="") && (_wpic!="")) or (_wCName iskindof "C_Plane_Civil_01_F")) then {
			/*
		...
		*/
		...
		if ((_PARAM_FIRAIR == 1) and ((_wCName == "FIR_F16C") or (_wCName == "FIR_F15C") or (_wCName == "FIR_F15D") or (_wCName == "FIR_F15E") or (_wCName == "FIR_F15K") or (_wCName == "FIR_A10A") or (_wCName == "FIR_A10C"))) then {
			if !(_wDName in _namelist) then {
				_vehicleList pushback [_wCName,_wDName,_wPic,_wDesc];
				_namelist pushback _wDName;
			};
