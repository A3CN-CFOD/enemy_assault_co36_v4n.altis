...
if (_PARAM_TASKTYPES == 1) then {
	...
	_objlist = ["leader","rescue","ARTY","dataterminal","ammo","intel","ZSU_AA2","leader2","dataterminal2","Acquireobj","intel2","rescue2","ARTY2","ZSU_AA","ammo2","intel3","leader3","ammo3","Bombtruck","dataterminal3","leader4","ARTY3"];//"leader","rescue","ARTY","ammo","intel","ZSU_AA2","leader2","Acquireobj","intel2","rescue2","ZSU_AA","ammo2","intel3","leader3","ammo3","Bombtruck","leader4","ARTY2"
};
...
for "_o" from 1 to (_numobjs) do {
	...
	switch (_objsel) do
		{   
			...
			[_random_pos,_radarray,_acquiredobjsel,(getmarkerpos "object_drop_point"),(_PARAM_AISkill/10),_locselname,[true,"ColorRed",[200,200]],_ghst_side] call ghst_fnc_randomacquire;
			};
		};
	sleep 3;
};
...
While {true} do {
	...
	_randnum = (random 10);
	sleep 10;
	if ((_ghst_side countSide allunits < 35) and {(_randnum > 7)} and {(count _curtasks > 0)} and {(Time > _timedelay)}) then {
	...
	if ((_ghst_side countSide allunits < 35) and {(_randnum < 3)} and {(count _curtasks > 0)} and {(_max_reinf < _numobjs)} and {(Time > _timedelay)}) then {
