...
//Put specified objects in Buildings With Guards
{
	...
	private ["_loop","_p","_selbuild","_position","_positionarray","_pos","_eGrp","_g","_selunit","_tsk","_zadjust"];
	...
	while {_loop} do {
		private ["_n","_i","_r"];
		if (count _nearBuildings isEqualTo 0) exitwith {_loop = false};
		...
		//get positions for selected building
			if !(_selbuild iskindof "Piers_base_F") then {
				_positionarray = _selbuild buildingPos -1;//[_selbuild] call BIS_fnc_buildingPositions;
			} else {
				_positionarray = [];
			};
		//_positionarray = _selbuild call ghst_fnc_build_positions;
	...
	if (_markunits) then {
		...
		[_selunit,_mark1,_selbuild,_tsk] spawn {
			...
			while {alive _unit} do {
				...
				if (((damage _selbuild) > 0.9) and {!(_unit iskindof "Land_Suitcase_F")} and {!(_unit iskindof "Land_DataTerminal_01_F")}) exitwith {_unit setdamage 1;};
