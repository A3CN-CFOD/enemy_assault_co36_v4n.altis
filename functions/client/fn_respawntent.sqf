//V1 Script by: Ghost - calls in a support Respawn tent
...
if !(isnil "ghst_respawntent_pdata") then {
	...
	//_ghst_respawntent_data set [_idx,-1];
	//_ghst_respawntent_data = _ghst_respawntent_data - [-1];
...
_tent = _type createvehicle [_pos select 0, _pos select 1, 0];

_tent setdir _dir;
_tent setposatl [_pos select 0, _pos select 1, 0];
_tent setVectorUP (surfaceNormal [_pos select 0,_pos select 1]);
...
player setVariable ["ghst_respawntent", false];
